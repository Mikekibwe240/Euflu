...
<div class="max-w-6xl mx-auto mt-10 mb-8 px-2 sm:px-4 md:px-6 lg:px-0">
    <div class="text-2xl md:text-4xl font-extrabold text-white uppercase tracking-wider mb-2">
        JOURNÉE <span class="text-[#6fcf97]"><?php echo e(request('journee', $currentJournee ?? 1)); ?></span>
    </div>
    ...
    <div class="space-y-3 md:space-y-4">
        ...
        <a href="<?php echo e(url('/matchs/' . $match->id)); ?>" class="block group">
            <div class="grid grid-cols-1 md:grid-cols-8 items-center bg-[#181d1f] rounded-lg shadow px-2 md:px-6 py-3 md:py-4 border-l-4 border-[#6fcf97] gap-2 md:gap-4 transition duration-200 group-hover:bg-[#23272a] group-hover:shadow-lg group-hover:border-[#e2001a] cursor-pointer overflow-x-auto md:overflow-x-visible min-w-0">
                <div class="text-gray-400 text-xs font-bold uppercase md:col-span-1 col-span-full mb-2 md:mb-0"><?php echo e(\Carbon\Carbon::parse($match->date)->locale('fr')->translatedFormat('l d F')); ?></div>
...
<?php endif; ?>
                ...
                <div class="text-xs md:text-base text-[#6fcf97] text-center"><?php echo e($match->mvp ?? '-'); ?></div>
    ...
    <div class="flex justify-center mt-4 gap-4">
        <?php if(isset($prevJournee) && $prevJournee): ?>
            <a href="?journee=<?php echo e($prevJournee); ?>&saison_id=<?php echo e(request('saison_id', $saisons->first()?->id)); ?>" class="px-8 py-3 bg-[#23272a] border-2 border-[#6fcf97] text-white font-bold rounded hover:bg-[#6fcf97] hover:text-[#23272a] transition">&larr; MATCHDAY <?php echo e($prevJournee); ?></a>
        <?php endif; ?>
        <?php if(isset($nextJournee) && $nextJournee): ?>
            <a href="?journee=<?php echo e($nextJournee); ?>&saison_id=<?php echo e(request('saison_id', $saisons->first()?->id)); ?>" class="px-8 py-3 bg-[#23272a] border-2 border-[#6fcf97] text-white font-bold rounded hover:bg-[#6fcf97] hover:text-[#23272a] transition">MATCHDAY <?php echo e($nextJournee); ?> &rarr;</a>
        <?php endif; ?>
    </div>
</div>
