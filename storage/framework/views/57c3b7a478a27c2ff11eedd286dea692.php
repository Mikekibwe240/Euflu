...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-xl p-8 border border-blue-100 dark:border-blue-800">
        <div class="mb-4 flex items-center gap-4">
            <span class="inline-block bg-blue-600 text-white rounded-full px-4 py-1 font-bold text-lg shadow">N° <?php echo e($article->id); ?></span>
            <h1 class="text-3xl font-extrabold text-blue-800 dark:text-blue-200"><?php echo e($article->titre); ?></h1>
        </div>
        <div class="mb-2 text-gray-500 dark:text-gray-400 text-sm flex flex-wrap gap-2 items-center">
            <span class="bg-blue-100 dark:bg-blue-800 text-blue-700 dark:text-blue-200 px-2 py-1 rounded text-xs font-semibold">Saison : <?php echo e($article->saison->nom ?? $article->saison->annee ?? '-'); ?></span>
            <span class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 px-2 py-1 rounded text-xs">Auteur : <?php echo e($article->user->name ?? '-'); ?></span>
            <span class="bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-200 px-2 py-1 rounded text-xs">Publié le : <?php echo e($article->created_at->format('d/m/Y')); ?></span>
        </div>
        ...
        <?php if($article->video): ?>
            <div class="mb-4 flex justify-center">
                <video controls class="w-full max-w-lg rounded-xl shadow cursor-pointer" onclick="openMediaModal('video', '<?php echo e(asset('storage/' . $article->video)); ?>')">
        ...
        <?php endif; ?>
        <?php if($imgCount > 1): ?>
            <div id="carousel-admin-<?php echo e($article->id); ?>" class="relative mb-4 group">
                <div class="overflow-hidden rounded-lg shadow h-64 relative">
                    <?php $__currentLoopData = $article->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <img src="<?php echo e(asset('storage/' . $img->path)); ?>" alt="Image article" class="w-full h-64 object-cover absolute inset-0 transition-all duration-700 ease-in-out <?php echo e($i === 0 ? '' : 'hidden'); ?> cursor-pointer" data-carousel-item onclick="openMediaModal('image', '<?php echo e(asset('storage/' . $img->path)); ?>')" />
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
                <button type="button" aria-label="Précédent" class="absolute top-1/2 left-2 -translate-y-1/2 bg-white/90 hover:bg-blue-100 dark:bg-gray-900/80 rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" onclick="carouselPrevAdmin(<?php echo e($article->id); ?>)">
                ...
                <button type="button" aria-label="Suivant" class="absolute top-1/2 right-2 -translate-y-1/2 bg-white/90 hover:bg-blue-100 dark:bg-gray-900/80 rounded-full p-2 shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10" onclick="carouselNextAdmin(<?php echo e($article->id); ?>)">
                ...
                <div class="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2 z-10">
                    <?php $__currentLoopData = $article->images; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $img): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <button type="button" aria-label="Aller à l'image <?php echo e($i+1); ?>" class="w-3 h-3 rounded-full border-2 border-blue-400 bg-white transition-all duration-300" style="opacity:<?php echo e($i === 0 ? '1' : '0.5'); ?>" data-carousel-indicator onclick="carouselGoToAdmin(<?php echo e($article->id); ?>, <?php echo e($i); ?>)"></button>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </div>
            </div>
        <?php elseif($imgCount === 1): ?>
            <div class="mb-4 flex justify-center">
                <img src="<?php echo e(asset('storage/' . $article->images->first()->path)); ?>" alt="Image principale" class="rounded-xl max-w-full h-64 border border-gray-200 dark:border-gray-700 bg-white shadow" onerror="this.style.display='none'" />
            </div>
        <?php endif; ?>
        <div class="flex gap-2 mb-6">
            <a href="<?php echo e(route('admin.articles.edit', $article)); ?>" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded shadow">Modifier</a>
            ...
            <a href="<?php echo e(route('admin.articles.index')); ?>" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded shadow ml-auto">← Retour à la liste</a>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    <?php if($imgCount > 1): ?>
        showCarouselItemAdmin(<?php echo e($article->id); ?>, 0);
    <?php endif; ?>
});
</script>
<?php $__env->stopSection(); ?>
