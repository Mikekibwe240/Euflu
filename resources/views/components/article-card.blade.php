...
<div class="bg-[#23272a] p-0 flex flex-col overflow-hidden {{ $class }} cursor-pointer group" onclick="window.location='{{ route('public.articles.show', $article->id) }}'">
    ...
    @if($showMedia)
        ...
        @elseif($imgCount > 1)
            <div class="relative w-full {{ $imgHeight }} group" data-carousel>
                <div class="overflow-hidden w-full h-full relative">
                    @foreach($article->images as $i => $img)
                        <div data-carousel-item class="w-full h-full {{ $imgHeight }} {{ $i === 0 ? '' : 'hidden' }}">
                            <img src="{{ asset('storage/' . $img->path) }}" alt="Image article" class="w-full h-full object-cover cursor-pointer" onclick="event.stopPropagation();openMediaModal('image', `{{ asset('storage/' . $img->path) }}`)" />
                        </div>
                    @endforeach
                </div>
