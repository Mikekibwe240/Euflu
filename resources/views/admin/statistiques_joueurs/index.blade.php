...
<div class="container mx-auto p-4">
    <h2 class="text-2xl font-bold mb-4">Statistiques Joueurs</h2>
    <form method="GET" action="{{ route('statistiques_joueurs.index') }}" class="mb-4 flex flex-wrap gap-4 items-end">
        <div>
    ...
    <table class="min-w-full bg-white rounded shadow">
        ...
        <tbody>
            @forelse($stats as $stat)
                <tr class="border-t">
                    <td class="px-4 py-2">{{ $stat->joueur->nom }} {{ $stat->joueur->prenom }}</td>
                    <td class="px-4 py-2">{{ $stat->joueur->equipe->nom ?? '-' }}</td>
                    <td class="px-4 py-2">{{ $stat->buts }}</td>
                    <td class="px-4 py-2">{{ $stat->cartons }}</td>
                    <td class="px-4 py-2">{{ $stat->mvp }}</td>
                </tr>
            @empty
                <tr><td colspan="5" class="text-center py-4">Aucune statistique trouvée.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
