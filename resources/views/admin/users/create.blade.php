...
<div class="container mx-auto p-4">
    ...
    @endif
    <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-4 bg-bl-card border border-bl-border p-6 rounded-xl shadow-lg text-bl-accent" autocomplete="off">
        @csrf
        <div>
            <label for="name" class="block font-semibold mb-1 text-white">Nom</label>
            <input type="text" name="name" id="name" class="form-input w-full bg-bl-dark border-bl-border rounded-lg text-white" value="{{ old('name') }}" required>
            @error('name')<div class="text-red-600 text-sm">{{ $message }}</div>@enderror
        </div>
        <div>
            <label for="email" class="block font-semibold mb-1 text-white">Email</label>
            <input type="email" name="email" id="email" class="form-input w-full bg-bl-dark border-bl-border rounded-lg text-white" value="{{ old('email') }}" required>
