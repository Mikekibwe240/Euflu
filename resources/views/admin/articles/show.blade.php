...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <div class="bg-bl-card rounded-2xl shadow-xl p-8 border border-bl-border">
        <div class="mb-4 flex items-center gap-4">
            <span class="inline-block bg-bl-accent text-white rounded-full px-4 py-1 font-bold text-lg shadow">N° {{ $article->id }}</span>
            <h1 class="text-3xl font-extrabold text-white">{{ $article->titre }}</h1>
        </div>
        <div class="mb-2 text-gray-400 text-sm flex flex-wrap gap-2 items-center">
            <span class="bg-bl-card border border-bl-border text-white px-2 py-1 rounded text-xs font-semibold">Saison : {{ $article->saison->nom ?? $article->saison->annee ?? '-' }}</span>
            <span class="bg-bl-card border border-bl-border text-gray-200 px-2 py-1 rounded text-xs">Auteur : {{ $article->user->name ?? '-' }}</span>
            <span class="bg-bl-card border border-bl-border text-green-400 px-2 py-1 rounded text-xs">Publié le : {{ $article->created_at->format('d/m/Y') }}</span>
        </div>
        ...
        <div class="flex gap-2 mb-6">
            <a href="{{ route('admin.articles.edit', $article) }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded shadow border border-yellow-500 transition">Modifier</a>
            ...
            <a href="{{ route('admin.articles.index') }}" class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded shadow border border-yellow-500 ml-auto transition">← Retour à la liste</a>
        </div>
    </div>
</div>
@endsection
